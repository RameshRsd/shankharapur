...
//        $room = RoomCheck::where('checked_in_date','>=',date('Y-m-d'))->orderBy('id','DESC');
    ...
    public function continueCheck($id){
        ...
        $guests = Guest::orderBy('first_name')->get();
        $previousPage=url('admin/work-flows/room-check');
        return view('backend.admin.work.checking.createContinue',compact('title','heading','roomTypes','guests','previousPage','roomCheck'));
    }
    public function continueCheckStore(Request $request,$id){
        $this->validate($request,[
            'guest_id' => 'required',
            'room_id' => 'required',
            'checked_in_date' => 'required',
